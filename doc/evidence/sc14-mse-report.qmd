...
in subsequent MSE work. The sources have different roles:
...
\hline
\textbf{Working label} & \textbf{Legacy ID} & \textbf{HCR family} &
\textbf{Target (kt)} & \textbf{Trigger} & \textbf{Annual limits} \\
\hline
HS+20 & MP29 & Hockey-stick & 2,000 & 2.125 & -15\% / +20\% \\
HS-20 & MP43 & Hockey-stick & 2,000 & 3.0625 & -20\% / +15\% \\
HSsym & MP45 & Hockey-stick & 2,000 & 3.0625 & -15\% / +15\% \\
...
The Figure 4 quilt now supplies a reproducible worked scorecard for the eight
candidate variants under the reference OM. Let $x_{jm}$ be the value for CMP
$j$ and metric $m$, and let $x_m^{\min}$ and $x_m^{\max}$ be the minimum and
maximum across the included CMPs. The directionally normalized preference
score $p_{jm}$ is
...
reads the editable Figure 4 input, checks that every CMP has one value for every
included metric, and writes both the totals and metric-level contributions.

For the worked result below, all six Figure 4 metrics are included with equal
weight ($I_m=1$, $w_m=1$): spawning biomass, fishing pressure, catch, IACC,
vulnerable biomass relative to 2025, and vulnerable biomass relative to its
MSY-stock-level reference. All inputs use the reference OM and the 2041--2050
summary window used in Figure 4.
...
These values are a reproducible calculation, but their order is still
**illustrative rather than a management ranking**: equal weights are a neutral
starting point, not agreed priorities, and the scores are relative to this
candidate set.
