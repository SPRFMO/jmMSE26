...
This distinction matters for interpreting the illustrative scorecard.
**PRsym** ranks first under both displayed weighting schemes, but the ordering
below it changes materially with weights: **PR-20** is second with equal
weights, whereas **HS-30** is second when Catch and IACC jointly receive half
of the total weight. Thus the scorecard does not identify a weight-independent
winner. The prominence of HS+20 and PR+20 reflects their role as initial design
anchors, not their scorecard rank.
...
the interpretation of two-stock robustness; the use of final 500-posterior
...
|---|---|---|---:|---:|---:|
| HS+20 | MP29 | Hockey-stick | 2,000 | 2.3125 | -15% / +20% |
| HS-20 | MP43 | Hockey-stick | 2,000 | 2.3125 | -20% / +15% |
| HSsym | MP45 | Hockey-stick | 2,000 | 2.3125 | -15% / +15% |
| HS-30 | MP47 | Hockey-stick | 2,000 | 2.3125 | -30% / +20% |
| PR+20 | MP32 | Power ramp | 1,500 | 1.703125 | -15% / +20% |
| PR-20 | MP44 | Power ramp | 1,500 | 1.65625 | -20% / +15% |
| PRsym | MP46 | Power ramp | 1,500 | 1.6796875 | -15% / +15% |
| PR-30 | MP48 | Power ramp | 1,500 | 1.609375 | -30% / +20% |
...
::: {.callout-note title="500-posterior tuning result"}
All eight procedures were tuned against the same 500 posterior iterations of
the reference OM. The four hockey-stick variants converged to trigger 2.3125.
The achieved 2041--2050 Kobe-green probabilities for HS-20, HSsym, and HS-30
were each 0.609; the base HS+20 run also met the 0.60 objective within the
specified 0.01 tolerance. The power-ramp triggers ranged from 1.609375 to
1.703125; achieved probabilities for PR-20, PRsym, and PR-30 were 0.6054,
0.6000, and 0.5912. Shared or nearby trigger values reflect the discrete
bisection tolerance and common posterior sample, not an assumption that annual
TAC-change limits have no effect.
:::
...
The results and candidate summaries in this report use the completed
500-posterior tuning and evaluation runs. They supersede the preliminary
100-iteration comparison outputs.
...
They should therefore be compared as complete, retuned CMPs. The completed
500-posterior reference and nine-OM robustness runs provide the common
evidence base for this report and the accompanying Slick file.
...
The SC14 report and `output/jm_candidates.slick` use one shared numerical
evidence layer. Both contain the same eight CMPs, reference OM, nine robustness
OMs, 500 posterior iterations, projection years, and six annual measures:
$SB/SB_{\mathrm{MSY}}$, $F/F_{\mathrm{MSY}}$, Catch, IACC,
$VB/VB_{2025}$, and $VB/VB_{\mathrm{MSY}}$. Slick retains iteration-level
interactive arrays; the report derives period summaries, figures, and the
illustrative scorecard from those same arrays. Narrative, citations, and
interpretive cautions are report-only content.
...
The reference-OM scorecard in @tbl-scorecard-results provides the current
eight-CMP comparison. PRsym ranks first under both the equal-weight and
balanced schemes. With equal weights, PR-20 and PR+20 rank second and third;
with the balanced weights, HS-30 and HS-20 rank second and third. HS+20 and
HSsym tie under both schemes because their long-term summary values are
identical at the displayed precision. PR-30 ranks eighth in both views.
...
comparison of fishing pressure and spawning-biomass status for the near-term
...
![Near- and long-term Kobe plots for the eight CMPs under the reference operating model.](../figures/candidates/kobe_reference_periods.png){#fig-kobe-reference-periods fig-align="center" width="95%" fig-alt="Two Kobe panels compare eight candidate management procedures. Fishing mortality relative to FMSY is on the horizontal axis and spawning biomass relative to SBMSY is on the vertical axis. Panels show 2026 to 2035 and 2041 to 2050."}
...
2036--2040. Points show means among 500 posterior iterations, and horizontal
and vertical bars show the separate 10th--90th percentile ranges for the two
measures.
...
catch. Points are means among 500 posterior iterations; horizontal and vertical bars are
...
The spawning-biomass figures in Sections 6.2 and 6.3 require four
interpretive cautions. First, the x and y uncertainty bars are
marginal percentiles and do not show the within-replicate covariance between
plotted measures. Second, spawning biomass is shown as a standardized
dynamic-reference-point ratio rather than in tonnes. Third, these views cover
...
values are based on 500 posterior iterations and are saved in
`doc/data/sc14-preliminary-tradeoffs.csv`; they can be
...
|---|---|---|
| Profile each trigger objective around the selected value | Quantifies sensitivity to the bisection tolerance | Eight CMPs retuned against one frozen 500-posterior reference OM |
| Finalize TAC-change comparison | Tests responsiveness and stability under management-relevant constraints | Eight retuned CMPs evaluated over the 500-posterior reference and nine robustness OMs |
