...
date: now
date-format: "D MMMM YYYY, h:mm A z"
...
workshop and subsequent model and management-procedure development.
:::
...
Two CMPs provide the central comparison. They are described as central because
they anchor the initial rule-shape comparison, not because the current results
rank them highest. The **+20** variants were specified in response to an
initial request arising from early feedback from Chile's internal MSE research
project; the 20% maximum annual TAC increase was not selected from the later
Commission poll. That poll subsequently helped inform the broader set of
annual TAC-change variants examined in this report.
...
This distinction matters for interpreting the preliminary scorecard. The six
retuned variants rank above the two initial **+20** configurations in
@tbl-scorecard-results under both illustrative weighting schemes. Four of the
six included metrics describe stock condition, so equal weighting assigns
two-thirds of the composite weight to those measures; the balanced scheme
assigns them half. Thus the scorecard ordering reflects the later retuning and
the selected metric set and weights, whereas the prominence of HS+20 and PR+20
reflects their role as the initial design anchors.

The original tuning record describes both as targeting an approximately 60%
mean probability of dynamic Kobe-green status during 2041--2050. However,
recalculation from the currently published 100-draw run objects gives 26.2%
for HS+20 and 27.3% for PR+20, compared with approximately 56--59% for the six
retuned variants. These archived **+20** runs are therefore not equally tuned
to the variants and should not be interpreted as a like-for-like tuned
comparison. A completed 500-draw refinement brings all eight candidates within
one percentage point of the 60% target; the report outputs should move to that
set only when its reference and robustness performance artifacts are adopted
together. In the comparisons reviewed on
...
the interpretation of two-stock robustness; the restored 100-replicate
results pending a validated higher-replication refinement; and the work needed
...
::: {.callout-caution title="Trigger values require a common-resolution check"}
The identical 3.0625 triggers for HS-20, HSsym, and HS-30 were produced by
separate tuning runs; the saved traces do not indicate that one value was
carried across candidates. Each bisection began with the same interval and
stopped at the first tested trigger whose simulated Kobe-green probability was
within 0.01 of the 0.60 objective. At 3.0625, the three probabilities were
0.606, 0.604, and 0.606, respectively. Their probabilities were also nearly
identical at the preceding bisection points. The shared displayed trigger
therefore reflects the tuning tolerance and resolution of these 100-replicate
runs; it does not demonstrate that the three exact trigger values are
identical.

The contrast with HS+20 remains important: its reported trigger is 2.125,
whereas the retuned hockey-stick variants first met the stated tolerance at
3.0625. Because that direction is not intuitively implied by the annual
TAC-change constraints alone, the reduced candidate set should be retuned and
the trigger--objective relationship explicitly profiled using one frozen OM,
software configuration, random-number design, and higher replication before
the triggers are treated as final.
:::
...
should use higher-replication results only after the CMP definitions and
annual-limit behavior have been independently validated.
...
$VB/VB_{\mathrm{MSY}}$, and dynamic Kobe-green probability are treated as
better; lower $F/F_{\mathrm{MSY}}$, IACC, dynamic Kobe-red probability,
...
The Kobe probabilities use the same dynamic biomass denominator as the tuning
objective: the equilibrium $SB_{\mathrm{MSY}}/SB_0$ fraction is applied to
projected unfished spawning biomass in each year and iteration. The low-catch
probability uses 2041--2050; mean catch reduction averages positive annual
percentage reductions during 2026--2050. These four diagnostics are displayed
in the quilt but are not automatically added to the illustrative composite
score below, avoiding an undocumented change in weights and partial
double-counting of stock status, fishing pressure, catch, and IACC.

![Reference-OM candidate performance metrics for the eight naming-convention variants. Cell labels are median iteration summaries, except the dynamic Kobe probabilities, which are means over all iteration-years. Purple indicates lower relative preference and light lavender indicates higher relative preference within each metric (0 is darkest; 1 is lightest).](../figures/candidates/candidate_quilt_reference.png){#fig-candidate-quilt fig-align="center" width="100%"}
...
The worked results in @tbl-scorecard-results include the established six
scorecard metrics from @fig-candidate-quilt; the four added diagnostic columns
are visible inputs with `include = FALSE` until their inclusion and weights are
reviewed. The equal-weight view assigns one-sixth to every included metric.
...
not the final scorecard. It contains eight CMPs and ten displayed performance
metrics, six of which are included in the worked composite scores.
Please note:
...
medians of those period means across the 100 iterations. The vertical and
horizontal lines at 1 define the conventional Kobe quadrants.

![Near- and long-term Kobe plots for the eight CMPs under the reference operating model.](../figures/candidates/kobe_reference_periods.png){#fig-kobe-reference-periods fig-align="center" width="95%" fig-alt="Two Kobe panels compare eight candidate management procedures. Spawning biomass relative to SBMSY is on the horizontal axis and fishing mortality relative to FMSY is on the vertical axis. Panels show 2026 to 2035 and 2041 to 2050."}
...
set rather than as feedback within the HCR. Preliminary 100-iteration results
are now available for HS+20 and PR+20 under the reference, low-recruitment, and
cyclic-recruitment OMs. The tested implementation used a 10% banking or
borrowing amount when unconstrained HCR advice changed by at least 15%, subject
to the CMP-specific healthy-rule condition. In operational terms, a decline of
more than 15% made an iteration eligible to **borrow** 10% of the new advice
(adding it to the current TAC and deducting it subsequently), while an increase
of more than 15% made it eligible to **bank** 10% (deducting it from the current
TAC and making it available subsequently). Eligibility was restricted to years
in which the combined abundance indicator was in the upper, or ``healthy'',
branch of the applicable HCR. For HS+20 this is rule 3: the indicator is at or
above the hockey-stick trigger and the unconstrained advice is on the constant
target branch. For PR+20 this is rule 2: the indicator is at or above the
power-ramp trigger and advice is on its upper branch. Thus, a change exceeding
15% was necessary but was not by itself sufficient. These runs did **not**
retain the annual TAC-change limits, so they do not isolate banking and
borrowing from the effect of removing those limits.

The contractor-reported long-term results are summarized in
@tbl-bb-sb0green. Under the reference OM, mean SB0green during 2041--2050
declined from 0.603 to 0.589 for HS+20 and from 0.607 to 0.585 for PR+20. The
reported banking-and-borrowing values under low recruitment were 0.530 and
0.617, respectively, and under cyclic recruitment were 0.419 and 0.539.

| OM | CMP | Without B&B | With B&B |
|:---|:---|---:|---:|
| Reference OM | HS+20 | 0.603 | 0.589 |
| Reference OM | PR+20 | 0.607 | 0.585 |
| Low recruitment | HS+20 | -- | 0.530 |
| Low recruitment | PR+20 | -- | 0.617 |
| Cyclic recruitment | HS+20 | -- | 0.419 |
| Cyclic recruitment | PR+20 | -- | 0.539 |

: Contractor-reported mean SB0green during 2041--2050 for the preliminary
100-iteration banking-and-borrowing sensitivity. A dash indicates that the
corresponding no-B&B comparison was not supplied in the contractor summary.
{#tbl-bb-sb0green}
...
the distribution across 100 iterations after first averaging SB/SBMSY within
...
![Spawning biomass relative to SBMSY in the preliminary banking-and-borrowing runs, for the near-term (2026--2035) and long-term (2041--2050) periods.](../figures/banking-borrowing-sbmsy-periods.png){#fig-bb-sbmsy-periods fig-align="center" width="95%" fig-alt="Two boxplot panels show iteration-level mean spawning biomass relative to SBMSY for HS plus 20 and PR plus 20 under the reference, low-recruitment, and cyclic-recruitment operating models. The left panel covers 2026 to 2035 and the right panel covers 2041 to 2050. A dashed horizontal line marks one."}

The transferred run archive requires an implementation check before these
values are treated as final. Recalculation from the supplied `babs.rds` gives
some different SB0green values, and its saved `banking.isys` and
`borrowing.isys` tracking fields contain no non-zero transactions. Inspection
of the archived implementation shows that the intended previous HCR advice is
retrieved for the current year, which makes the transaction test compare the
current value with itself. @fig-bb-eligibility therefore shows **reconstructed
eligibility events**, based on the intended comparison with the preceding
year's HCR advice; it must not be interpreted as a count of transactions
actually applied in the archived runs.

::: {.content-visible when-format="pdf"}
\newpage
:::

![Reconstructed annual eligibility for banking and borrowing in the preliminary 100-iteration runs. Circles denote iterations eligible for banking and triangles denote iterations eligible for borrowing.](../figures/banking-borrowing-eligibility.png){#fig-bb-eligibility fig-align="center" fig-pos="H" width="95%" fig-alt="Six-panel plot of reconstructed banking and borrowing eligibility counts by year. Rows show the reference, low-recruitment, and cyclic-recruitment OMs; columns show HS plus 20 and PR plus 20. Banking eligibility is more frequent for PR plus 20, while borrowing eligibility is relatively uncommon."}

Final advice should use a corrected rerun that retains explicit transaction
tracking and, if the intent is to estimate the incremental effect of banking
and borrowing, includes a matched comparison with the same TAC-change-limit
treatment.
...
index-based rules as sensible. Their comments were made at a particular stage
of the workshop process and should not be read as a complete account of
stakeholder engagement. Commission participants and other stakeholders had
received several presentations and opportunities to provide input before and
during the MSE process, including input on management preferences and annual
TAC-change limits. Suggestions for further engagement are therefore best
understood as support for continuing and documenting that involvement, not as
an indication that stakeholder input was absent.
...
- that the preliminary banking-and-borrowing results suggest effects of a
  manageable magnitude at the investigated levels, but require a corrected,
  matched rerun before implementation conclusions are drawn; and
...
|---|---|---|
| Retune the reduced set at higher replication and profile each trigger objective | Stabilizes final triggers and resolves the HS+20 versus retuned-HS discrepancy and the shared 3.0625 stopping value | Required under one frozen OM, software, and random-number configuration |
...
| Validate and complete banking-and-borrowing sensitivity | Tests an implementation provision outside the HCR | Preliminary contractor results available; transferred archive records no transactions and requires a corrected, matched rerun |
