...
- **PR-20 (MP44)** uses a power-ramp HCR, a 1,500-kt catch target, the same
  ...
  advice as the indicator declines.

Both use the combined indicator from five standardized abundance indices.
...
This distinction matters for interpreting the illustrative scorecard. With all
eight CMPs now tuned on the same 500 draws, HS+20 ranks first under both shown
weighting schemes, while PR+20 ranks seventh; the six variants occupy positions
throughout the ordering. That order is a relative trade-off result, not an
acceptability test. It depends on the selected metrics, directions, scaling,
and weights, and should be read alongside the raw outcomes and uncertainty.
The scorecard ordering does not determine which cases are focal: focal status
records the present analytical emphasis, whereas the scorecard summarizes
relative trade-offs under selected metrics and weights.
...
![Nested development of the focused CMP set from HCR family through target catch and tuned response to the resulting HS and PR cases. Trigger values in this development diagram are the earlier 100-draw controls; the current 500-draw refined controls are given in @tbl-candidate-variants.](../figures/candidates/nested_cmp_development.png){#fig-nested-cmp-development fig-align="center" width="100%"}
...
| HS-20 | MP43 | Hockey-stick | 2,000 | 1.9375 | -20% / +15% |
| HSsym | MP45 | Hockey-stick | 2,000 | 2.0625 | -15% / +15% |
| HS-30 | MP47 | Hockey-stick | 2,000 | 2.0000 | -30% / +20% |
| PR+20 | MP32 | Power ramp | 1,500 | 1.640625 | -15% / +20% |
| PR-20 | MP44 | Power ramp | 1,500 | 1.6015625 | -20% / +15% |
| PRsym | MP46 | Power ramp | 1,500 | 1.6250 | -15% / +15% |
| PR-30 | MP48 | Power ramp | 1,500 | 1.578125 | -30% / +20% |
...
under one frozen OM and common random-number configuration. The resulting
...
selected control using the same frozen 500 draws. The steps were 0.0625 for
...
1. **Raw performance and uncertainty.** Report the performance statistic in
   ...
   appropriate interval. This is the primary evidence layer and includes, for
...
The companion Slick Spider (radar) view applies the same higher-is-better
orientation to the six included scorecard metrics and repeats the 0--1
...
acceptability scores**: neither weighting scheme is an agreed statement of
...
current eight-CMP comparison. HS+20 ranks first and HSsym second under both the
equal-weight and balanced schemes. PR-20 ranks third under both. HS-30 and
PRsym exchange fourth and fifth positions between the schemes, followed by
HS-20, PR+20, and PR-30. The separation between the two **+20** sensitivity
CMPs shows why an analytical role should not be confused with a shared
performance ranking.
...
not evidence that either CMP is uniformly better. Interpretation is also
...
![Near-term catch versus VB/VBMSY across robustness operating models, 2026--2035. CJM points pool the five single-stock robustness OMs; Northern and Southern points separately pool the corresponding components of the four two-stock OMs.](../figures/candidates/catch_vbmsy_robustness.png){#fig-catch-vbmsy-robustness fig-align="center" width="90%"}
...
![Long-term catch versus VB/VB2025 across robustness operating models, 2041--2050. CJM points pool the five single-stock robustness OMs; Northern and Southern points separately pool the corresponding components of the four two-stock OMs.](../figures/candidates/catch_vb2025_robustness.png){#fig-catch-vb2025-robustness fig-align="center" width="90%"}
...
|---|---|---|
| Archive and independently reproduce the completed 500-draw tuning traces | Confirms the common tuning objective and preserves an auditable record of the refined controls | Eight CMPs refined under one frozen OM and common random-number configuration; archive review remains |
| Review the TAC-change comparison and document the shortlist | Connects the completed responsiveness and stability results to the alternatives advanced for Commission consideration | Common 500-draw reference and nine-OM robustness results available; interpretation and shortlist decision remain |
...
| Finalize metric definitions, periods, and absolute targets where defensible | Prevents ambiguous or directionally incorrect scoring | Three-layer framework documented; management targets and tolerances remain to be agreed |
| Sensitivity-test the scorecard | Shows how weights, candidate membership, and relative scaling affect the illustrative ordering | Reproducible relative-preference calculation available; sensitivity analysis remains |
